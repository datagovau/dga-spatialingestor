...
for resource in dataset['resources']:
	...
	if ("kml" in resource['format'].lower() or "kmz" in resource['format'].lower()) and 'geoserver' not in resource['url']:
		print resource
		kml_resources += [resource]
	if ("shp" in resource['format'].lower() or "shapefile" in resource['format'].lower()) and 'geoserver' not in resource['url']:
		print resource
...
if len(ows_resources) > 0:
	...
	email("geodata processing started for "+dataset['title'], "Data modified: " + str(parser.parse(data_modified_date)) + " New Dataset" + "\n" + "https://data.gov.au/api/action/package_show?id="+dataset['id'] + "\n" + "https://data.gov.au/dataset/"+dataset['name'])
...
if len(shp_resources) > 0 and dataset['name'] != 'city-of-hobart':
	...
	pargs = ['ogr2ogr','-f','PostgreSQL',"--config" ,"PG_USE_COPY","YES",'PG:dbname=\''+ db_settings['dbname']+'\' host=\''+db_settings['host']+'\' user=\''+db_settings['user']+ '\' password=\''+db_settings['password']+'\'' 
		,tempdir,'-lco','GEOMETRY_NAME=geom',"-lco", "PRECISION=NO",'-nln',table_name,'-a_srs', nativeCRS,'-nlt' ,'PROMOTE_TO_MULTI'] 
        #,'POLYGON']
	#pprint(pargs)
...
if psql:
	...
	if using_kml:
		try:
			cur.execute('alter table "'+table_name+'" DROP "description" RESTRICT, DROP timestamp RESTRICT, DROP begin RESTRICT, DROP "end" RESTRICT, DROP altitudemode RESTRICT, DROP tessellate RESTRICT, DROP extrude RESTRICT, DROP visibility RESTRICT, DROP draworder RESTRICT, DROP icon RESTRICT;')
		except Exception,e:
			print e
			pass
	cur.execute('SELECT ST_Extent(geom) as box,ST_Extent(ST_Transform(geom,4326)) as latlngbox, ST_AsGeoJSON(ST_Extent(ST_Transform(geom,4326))) as geojson from "'+table_name+'"')
	...
	r = requests.put(geoserver_addr+'rest/workspaces/'+workspace+'/datastores', data=dsdata, headers={'Content-type': 'application/json'}, auth=(geoserver_user, geoserver_passwd))
	pprint(r)
	#echo r.status_code
	#echo r.text
...
if bbox:
	...
	(llminx,llminy, llmaxx, llmaxy) = latlngbbox.replace("BOX","").replace("(","").replace(")","").replace(","," ").split(" ")
...
for format in []:
      url = ws_addr+"wms?request=GetMap&layers="+layer_name+"&bbox="+bbox_obj['minx']+","+bbox_obj['miny']+","+bbox_obj['maxx']+","+bbox_obj['maxy']+"&width=512&height=512&format="+urllib.quote(format)
...
if "wms" not in existing_formats:
	ckan.call_action('resource_create',{"package_id":dataset['id'],"name":dataset['title'] + " - Preview this Dataset (WMS)","description":"View the data in this dataset online via an online map","format":"wms",
	    "url":ws_addr+"wms?request=GetCapabilities", "wms_layer": layer_name, "last_modified": datetime.now().isoformat()})
else:
